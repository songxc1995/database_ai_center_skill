...
判据是**上下界分别解释**:缺的那段月份,有没有一个「不是缺口」的理由?

- **下界**:从 1 月起 —— 或者这就是序列的**起点年**(之前本来就没有数据)
- **上界**:到 12 月止 —— 或者这就是**当前年**且**收在当月附近**(留一个月余量:当月账期
  可能还没出账)。★ 不能只凭"是今年"就放行:那样今年 3 月就断采、缺了 4–9 月的序列,
  会被说成"年还没过完" —— 真缺口被吸收成正常在途

两头都有解释才不是缺口;**任何一头解释不了,就是真的少了账期**。

| reason | 含义 | 该做什么 |
|---|---|---|
| `series_start` | 起点年,连续收在 12 月 | 没事,**永远不会补齐**,别等 |
| `year_in_progress` | 当前年,连续从 1 月起 | 没事,会自己补齐 |
| `series_start_in_progress` | **同时是**起点年和当前年(今年年中才接入) | 没事;早于起点的月份永不补齐,年内剩下的会补上 |
| `awaiting_final_cycle` | **前一年**只差 12 月,现在还在 1–2 月(下界同样按 `lo_ok` 判,年中接入的部署也算) | 多半只是账单还没出,**先别查**;过了这个窗口它会变成 `missing_months` |
| **`missing_months`** | 其余一切 | **★ 账单数据缺口,去查** |

★ 标签按**实际用到了哪条放宽**贴,不按"哪个标志为真":一个只有今年一年、从 1 月起的序列
既是起点年也是当前年,但下界压根没用到"起点年"这条豁免 —— 标成 `series_start_in_progress`
等于说"早于起点的月份永不补齐",而它根本没有更早的月份。

★ `series_start_in_progress` 这一档是补出来的:早先写成「当前年且从 1 月起」和「起点年且到
12 月止」两条各管一头的规则,当一年**同时是**两者时(今年年中接入的部署,在它的第一个自然年内)
两条都不命中,于是掉进兜底被报成账单缺口 —— 一个完全正常的新部署,让人白查一趟。
本项目自己就差点是这个形状:数据起点 2021-08。

判据用的是**月份号的集合**而不是月数 —— 只数数量分不出「8-12 连续」和「8,9,11,12 有洞」,
而后者才是真缺口。年份也按**排序后**判定,不看服务端的返回顺序:位置式的判定在倒序返回时会把
`series_start` 和 `year_in_progress` 直接对调,两个都错、方向还相反。

★ **`years` 里有某年而 `months` 里一个月都没有,是最极端的账单缺口** —— 它现在标
`months_covered: 0` + `missing_months`。此前这种情况**连 partial 都不标**,读起来跟"完整"
一模一样:缺口最大的那种反而看不见。

★ 响应里**根本没有 `months` 序列**时(旧平台或摘要模式),不假装判定过:每一年带
`coverage_unknown: true` 和原因。**"查不了"和"没问题"必须是两种可见的答案。**
