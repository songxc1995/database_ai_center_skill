...
| reason | 判据 | 该做什么 |
|---|---|---|
| `series_start` | **最早**的一年,月份连续且**收在 12 月** | 没事,数据起点,**永远不会补齐**,别等 |
| `year_in_progress` | 年份等于**当前年**,月份连续且**从 1 月起** | 没事,会自己补齐 |
| **`missing_months`** | 其余一切:中间有洞、整年一个月都没有、或序列停在某个过去的年份 | **★ 账单数据缺口,去查** |

判据用的是**月份号的集合**而不是月数 —— 只数数量分不出「8-12 连续」和「8,9,11,12 有洞」,
而后者才是真缺口。首/末年也按**排序后**的年份判定,不看服务端的返回顺序:位置式的判定在倒序
返回时会把 `series_start` 和 `year_in_progress` 直接对调,两个都错、方向还相反。
