...
6. An ID is not a symptom. For “告警 N 的根因” use `alert-evidence --alert-id N` — its `ai_results` carry the recorded root cause. Semantic search cannot match a number: a query built around one returns unrelated weak hits or nothing, and "nothing similar is on record" would then be the wrong answer. A ticket number from another system is not indexed by the platform at all — say the platform has no record of that ID, not that there is no similar history.
